	...
	fetchOffsets := func(ctx context.Context) (offsets map[int32]kgo.Offset, err error) {
		...
		for _, part := range b.parts {
			if _, ok := offsets[part]; ok {
				...
				offsets[part] = kgo.Offset(b.cfg.Kafka.ConsumerResetOffset)
				level.Info(b.logger).Log("msg", "no committed offset found for partition, consuming from consumer reset fallback", "consumer_group", b.cfg.Kafka.ConsumerGroup, "part", part, "offset", offsets[part].String())
			}
		}
		return offsets, nil
	}
...
func (cfg *KafkaConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.Var(newKafkaOffsetVar(kgo.NewOffset().AtStart(), &cfg.ConsumerResetOffset), prefix+"kafka.consumer-reset-offset", "The offset to start consuming from if a partition has no commits.")
...
func newKafkaOffsetVar(val kgo.Offset, p *kafkaOffset) *kafkaOffset {
	*p = (kafkaOffset)(val)
	return p
}
